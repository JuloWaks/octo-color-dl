...
def train(train_loader, model, criterion, optimizer, epoch, _run):
  ...
  batch_time, data_time, losses = AverageMeter("train.batch_time"), AverageMeter("train.date_time"), AverageMeter("train.loss")

  end = time.time()
  for i, (input_gray, input_ab, target) in enumerate(train_loader):
    
    # Use GPU if available
    if _run.config.get("use_gpu", False): input_gray, input_ab, target = input_gray.cuda(), input_ab.cuda(), target.cuda()
...
def validate(val_loader, model, criterion, save_images, epoch,_run):
  ...
  batch_time, data_time, losses = AverageMeter("validate.batch_time"), AverageMeter("validate.date_time"), AverageMeter("validate.loss")
